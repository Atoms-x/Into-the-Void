...
void CBullet::SmokeFX() {
  ...
  for (int i = 0; i < count; i++) {
    ...
    //Trails the origin point of the smoke directly behind the bullet
...
void CBullet::UpdateFramenumber() {
...
/// particle effect (unless hitting an ant), and then dying.  
...
void CBullet::CollisionResponse(const Vector2& norm, float d, CObject* pObj){
  ...
  if(!m_bDead){
    if (!pObj) {
      m_pObjectManager->create(eSprite::Explosion, m_vPos);
      m_bDeadCheck = true;
